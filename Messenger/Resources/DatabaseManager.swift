...
/// Manager object to read and write data to real time firabese database
...
extension DatabaseManager {
    ...
    ///- `completion`: Async clousure to return with result
            ...
            let messages: [Message] = value.compactMap { (dictionary) in
                ...
                if type == "photo" {
                    ...
                    guard let videoUrl = URL(string: content),
                        let placeholder = UIImage(systemName: "video.circle") else { return nil }
                    ...
                    guard let longitud = Double(locationComponent[0])  ,
                        let latitude = Double(locationComponent[1]) else {
                            return nil
                    }
                    
                    let location = Location(location: CLLocation(latitude: latitude, longitude: longitud), size: CGSize(width: 300, height: 300))
